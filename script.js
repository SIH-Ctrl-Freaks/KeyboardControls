...
window.addEventListener('keydown', function(e) {
    for(let i = 0; i < keys.length; i++) {
        if(e.key == keys[i].getAttribute('keyname' ) || e.key == keys[i].getAttribute('lowerCaseName')) {
            ...
            if (e.key === 'w_key') {
                // Define what the W key does when pressed
                console.log('W key pressed');
            } else if (e.key === 'a_key') {
                // Define what the A key does when pressed
                console.log('A key pressed');
            } else if (e.key === 's_key') {
                // Define what the S key does when pressed
                console.log('S key pressed');
            } else if (e.key === 'd_key') {
                // Define what the D key does when pressed
                console.log('D key pressed');
            } else if (e.code === 'Space') {
                // Define what the SPACEBAR key does when pressed
